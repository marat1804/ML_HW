...
def choose_class(y_p, y_true, percent):
    if percent:
        ...
        top_elements = [i[0] for i in top_elements]
...
def accuracy_score(y_true, y_pred, percent=None):
    tp, fp, tn, fn = choose_class(y_pred, y_true,  percent)
    return (tp + tn)/(tp + tn + fp + fn)


def precision_score(y_true, y_pred, percent=None):
    tp, fp, tn, fn = choose_class(y_pred, y_true,  percent)
    return tp / (tp + fp)


def recall_score(y_true, y_pred, percent=None):
    tp, fp, tn, fn = choose_class(y_pred, y_true,  percent)
    return tp / (tp + fn)


def lift_score(y_true, y_pred, percent=None):
    tp, fp, tn, fn = choose_class(y_pred, y_true,  percent)
    return (tp / (tp + fp)) / ((tp + fn) / (tp + tn + fp + fn))


def f1_score(y_true, y_pred, percent=None):
    tp, fp, tn, fn = choose_class(y_pred, y_true,  percent)
    recall = tp / (tp + fn)
    precision = tp / (tp + fp)
